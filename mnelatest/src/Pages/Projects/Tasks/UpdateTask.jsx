...
import UpdateTasksAction from '../../../Redux/Actions/Task/UpdateTask';
...
export default function UpdateTask({tid,handleUpdate}) {
    ...
    const [user, setUser] = React.useState(task?.user_id);
    const [formData,setFormData]=useState({})
    const [title,setTitle]=useState(task?.title)
    const [priority,setPriority]=useState(task?.priority)
    const [estimatedCost,setEstimatedCost]=useState(task?.estimated_cost)
    const [startDate, setStartDate] = React.useState(task?.start_date);
    const [endDate, setEndDate] = React.useState(task?.due_date);
    const [isUpdate,setIsUpdate]=useState(false)
    const [description, setDescription] = React.useState(task?.start_date)
    ...
    const onInputChange = (e) => {
      ...
      const postMilestone=()=>{
        ...
        dispatch(UpdateTasksAction(tid,data))
        handleUpdate(updatedMilestone,tid)
       
      }
      console.log("task",task)
       React.useEffect(() => {
        dispatch(GetSingleTaskAction(tid))
